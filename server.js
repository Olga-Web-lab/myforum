...
app.get("/", urlencodedParser, async function (request, response) {
    ...
    var channels = await require('./module/Channel').getAllChannels();

    return response.render("channels",{
        request: request,
        response: response,
        channels: channels
    })
...
app.post("/", urlencodedParser, async function (request, response) {
    if(request.body.exit){
        request.session.isAutorized = false;
        return response.redirect("/");
    }
    return response.redirect("/addChannel");
});

app.get("/addChannel", urlencodedParser, function (request, response) {
    let errors = [];

    return response.render('addChannel', {
        request: request,
        response: response,
        errors: errors
    });
...
app.post("/addChannel", urlencodedParser, function (request, response) {
    if(request.body.nameInput){
        require('./module/Channel').createChannel({
            name: request.body.nameInput,
            creator: request.session.login
        });
        return response.redirect('/');
...
app.get("/channels/:id", urlencodedParser, async function (request, response) {
    try {
        request.session.participant = await require('./module/Channel').getParticipantFlag({
            userLogin: request.session.login,
            _id: request.params.id
        });
    }
    ...
    try {
        var singleMeet = await require('./module/Channel').getSingleChannel(request.params.id);
    }
    catch (e) {
        var messages = [];
    }

    request.session.channelId = request.params.id;

    return response.render('currentChannel',{
        request: request,
        response: response,
        messages: singleMeet.messages,
        name: singleMeet.name
    });
});

app.post("/channels/:id", urlencodedParser, function(request, response) {
    if(request.body.join){
        require('./module/Channel').joinToTheChannel({
            channelId: request.session.channelId,
            userLogin: request.session.login
        });
    }
    else{
        require('./module/Channel').writeMessage({
            meetId: request.session.channelId,
            author: `${request.session.lastName} ${request.session.name}`,
            messageText: request.body.textInput

        });
    }
    return response.redirect(`/channels/${request.session.channelId}`);
});